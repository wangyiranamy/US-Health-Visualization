...
```{r}
causes$Population_Size <- demographics$Population_Size
```
...
total_death1 <- causes %>%
  select(-contains("Min")) %>%
  select(-contains("Max")) %>%
  select(-State_FIPS_Code, -County_FIPS_Code, -CHSI_County_Name, -CHSI_State_Name, -CHSI_State_Abbr, -Strata_ID_Number, -LCD_Time_Span, -Population_Size) %>%
