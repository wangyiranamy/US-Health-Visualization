...
summary_measure_state <- summary_measure_df1%>%
  ...
  mutate(ALE = mean(ALE), All_Death = mean(All_Death), Health_Status = mean(Health_Status), Unhealthy_Days=mean(Unhealthy_Days))
ggplot(summary_measure_state, aes(CHSI_State_Abbr,ALE))+
...
p <- plot_geo(summary_measure_state, locationmode = 'USA-states') %>%
  add_trace(
    z = ~ALE, text = ~ALE, locations = ~CHSI_State_Abbr,
    color = ~ALE, colors = 'Purples'
  ) %>%
  colorbar(title = "ALE") %>%
  layout(
